...
def validateInput(numWordles: "int", input: "str") -> "bool":
    """Validates user input by making sure its a string of length 6 with xo- chars
    ...
    # Validate length
    if ((6 * numWordles) + (numWordles - 1)) != len(input):
        return False, False
...
class wordleSolver:
    ...
    def setNextWord(self, word: str) -> None:
        """TODO
        """
        self.wordleGuessed = word
    ...
    def inputResult(self, allResults: "str"):
        ...
        for puzzle in self.puzzles:
            ...
            result: "str" = allResults[resIdx : resIdx + 6]
            resIdx = resIdx + 6
            # Process the result
...
def main():
    ...
    solnDictFileName: "str" = "empowordles-solns.txt"
    guessDictFileName: "str" = "empowordles-solns.txt"
    ...
    guesses = ["adores", "pollen"]
    results = ["xx-xox", "xoxxox"]
    # guesses = []
    # results = []
    guessIdx = 0
    ...
    while not ws.wordleSolved:
        if(guessIdx < len(guesses)):
            ws.setNextWord(guesses[guessIdx])
            print("Try " + ws.getNextWord())
            ws.inputResult(results[guessIdx])
            guessIdx = guessIdx + 1
        else:
            print("Try " + ws.getNextWord())
